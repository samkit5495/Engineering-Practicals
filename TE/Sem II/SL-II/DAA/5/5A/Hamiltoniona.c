...
#define max 50
static int c;
void op(int k,int a[][max],int v,int o)
{
	int i,j;
	for(i=1;i<=v;i++)
	{
...
void NextValue(int a[][max],int x[],int n,int k)
{
	...
	while(1)
	{
		...
		if(x[k]==0)
			return;
		if(a[x[k-1]][x[k]]!=0)//path exist from consecutive nodes in x array
		{
			for(j=1;j<k;j++)
				if(x[j]==x[k])
					break;
			if(j==k)
				if(k<n||(k==n&&a[x[n]][x[1]]!=0))//path exist from last to first node in x array
					return;
		}
	}
}
...
int main()
{
    ...
    for(i=1;i<=v;i++)
	{
		x[1]=i;
		c=1;
	    printf("Hamiltonion Path starting from:%d\n",i);
	    hamiltonion(a,x,v,2);
    }
    return 0;
}
